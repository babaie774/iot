...
const processShowProfileResponce = (resp:any):TranslatorResponce => {
    const {valid, outData, errors} = translateSchema(resp, resp.data, searchProfileData);
    if (valid) {
...
export const searchProfiles = async (params: {q: string}): Promise<TranslatorResponce> => {

  const resp: any = await serverBaseApi({
    method: "GET",
    url: `/profiles?q=${params.q}`,
  });
