import UserBadge from "basicComponents/userBadge";
...
function SideBarMenu({
  ...
  const [filteredUsers, setFilteredUsers] = useState<followingUsersTypes[]>([]);
  ...
  useEffect(() => {
    setFilteredUsers(sortedUsers.slice(0, offset));
  }, [sortedUsers, offset]);
    ...
    >
      ...
      <div className={styles.subMenus}>
        {subMenuItems(userId, isLogin).map((item, index) => {
          return (
            <Link href={item.link} key={index} passHref={true}>
              <div
                key={index}
